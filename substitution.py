...
def substitute_synonym_word(sent):
    ...
    while True and recusive_cnt <= 5:
        ...
        synonyms_pass_index_list = [idx for idx, score in enumerate(synonyms_score) if score < 0.8]
...
def substitute_homonym_char(sent, char_pronounce_dict):
    recusive_cnt = 0
    tmp_list = list(sent)
    while True and recusive_cnt <= 5:
        recusive_cnt += 1
...
def substitute_shape_char(sent, char_shape_dict):
    recusive_cnt = 0
    tmp_list = list(sent)
    while True and recusive_cnt <= 5:
        recusive_cnt += 1
